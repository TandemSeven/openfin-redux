...
export function* handleTakingClientSetValue(action) {
  if (action[SHARED_ACTION_ORIGIN_TAG] !== window[SHARED_ACTION_ORIGIN_TAG]) {
    ...
    // once createReactOpenfinMiddleware is imported, redux-openfin&react-openfin's
    // certain actions can be directly triggered by client redux.
    ...
    // use the redux for advanced features
    // once createReactOpenfinMiddleware is imported, redux-openfin&react-openfin's
    // certain actions can be directly triggered via client redux.
    yield put(applicationNewSnackbar({
...
export function* handleAppClosing(action) {
  console.log('client saga::handleAppClosing', action);
